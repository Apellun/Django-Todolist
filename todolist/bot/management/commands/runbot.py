...
class Command(BaseCommand):
    ...
    def get_boards(self, user_id: int) -> str:
        ...
        """
        boards = Board.objects.filter(boardparticipant__user=user_id, is_deleted=False)
        if len(boards) > 1:
    ...
    def create_goal(self, user: TgUser, client: TgClient, message: Dict) -> None:
        ...
        if message.text == "/stop":
            ...
            goal = TgUserNewGoal.objects.get(user=user)
            new_goal = Goal(category=goal.category, title=goal_title, user_id=user.user_id.id)
            new_goal.save()
            ...
            while True:
                ...
                for item in res.result:
                    ...
                    try:
                        user = TgUser.objects.get(telegram_chat_id=item.message.chat.id)
                            ...
                            tg_client.send_message(chat_id=item.message.chat.id,
                                                    text=f"Please verify your account. Enter the verification code into the app while logged in. Your verification code is {code}")
                    ...
                    if not user.verified:
                            tg_client.send_message(chat_id=item.message.chat.id, text=f"Please verify your account. Entrer the verification code into the app while logged in. Your verification code is {user.verification_code}")
                    elif item.message.text == "/start":
                        tg_client.send_message(chat_id=item.message.chat.id, text="Hello! Please input one of the following commands:\
